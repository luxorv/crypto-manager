...
class CryptoGame:
    def __init__(
        ...
        self.average_daily_erarnings = starting_cars * average_earning_per_car
    ...
    def calc_daily_earnings(self, current_week, current_day):
        running_cars = (len(self.unavailable_cars) + self.starting_cars)
        fuel_price = running_cars * self.fuel_price
        self.total_earnings += (self.average_daily_erarnings - fuel_price)
        ...
        for idx in range(len(self.unavailable_cars)):
            if idx <= len(self.unavailable_cars):
                ...
                if self.unavailable_cars[idx] == 0:
                    self.current_cars += 1
                    self.average_daily_erarnings += self.average_earning_per_car
                    self.unavailable_cars[idx] -= 1
                    print("Car bought now have {} with an average earnings of {} and {} current tokens". format(
                        self.current_cars,
                        self.average_daily_erarnings,
                        self.total_earnings
                    ))
    ...
    def calc_total_number_of_weeks(self):
        ...
        while True:
            ...
            for current_day in range(1, 8):
                self.calc_daily_earnings(current_week, current_day)
